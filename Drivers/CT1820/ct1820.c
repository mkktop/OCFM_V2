...
static uint8_t ct1820_read_byte(void)
{
    ...
    for (uint8_t i = 0; i < 8; i++) {
        DQ_MODER_OUT();
        DQ_L();
        ct1820_delay_us(2);     /* 保持低电平至少1μs */
        DQ_MODER_IN();
        ct1820_delay_us(8);     /* 等待总线稳定，在15μs内采样 */
        byte = (byte >> 1) | (DQ_READ() ? 0x80 : 0x00);
        ct1820_delay_us(50);    /* 等待时隙结束(至少60μs) */
    }
...
void CT1820_Init(void)
{
    ...
    g.Pin = CT1820_Pin;
    g.Mode = GPIO_MODE_OUTPUT_OD;   /* 开漏输出，配合外部上拉电阻 */
    g.Pull = GPIO_NOPULL;            /* 外部已有上拉电阻 */
